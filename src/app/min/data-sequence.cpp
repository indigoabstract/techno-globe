...
void data_sequence::set_read_position(uint64 position)
{
...
void data_sequence::set_write_position(uint64 position)
{
	if(position >= 0 && position <= get_size())
	{
...
void file_data_sequence::seek(uint64 ipos)
{
	file->io.seek(ipos);
	set_read_position(ipos);
	set_write_position(ipos);
}
